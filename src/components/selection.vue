  ...
  <v-card class="pa-5 d-flex flex-column align-stretch" style="height: 100%">
    ...
    <v-flex class="d-flex no-wrap justify-center align-stretch justify-space-between pa-5" grow style="width:100%">
      <v-flex class="d-flex flex-column align-center" xs3>
        <v-card style="height:100%; width: 100%" :class="`d-flex flex-column justify-center align-center ${phono.value ? 'primary' : 'grey darken-3'} custom`" :elevation="phono.value ? 5 : 0" @click="setSelection(phono.pin)">
      ...
      <v-flex class="d-flex flex-column align-center" xs3>
        <v-card style="height:100%; width: 100%" :class="`d-flex flex-column justify-center align-center ${feedBack.value ? 'primary' : 'grey darken-3'} custom`" :elevation="feedBack.value ? 5 : 0" @click="setSelection(feedBack.pin)">
      ...
      <v-flex class="d-flex flex-column align-center" xs3>
        <v-card style="height:100%; width: 100%" :class="`d-flex flex-column justify-center align-center ${lineIn.value ? 'primary' : 'grey darken-3'} custom`" :elevation="lineIn.value ? 5 : 0" @click="setSelection(lineIn.pin)">
          <img src="@/assets/line_in.png" style="height: 100px" />
          <span class="text-h5 mt-3">{{ lineIn.label }}</span>
        </v-card>
      </v-flex>
    </v-flex>
    <v-flex class="d-flex" v-if=" heatingStatus || speedStatus">
